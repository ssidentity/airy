...
func GetAffectedDeploymentsConfigmap(clientset kubernetes.Interface, configmapName string, namespace string, labelSelector string) ([]string, error) {
	...
	retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		for _, deploymentItem := range (*result).Items {
			...
			containers := callbacks.GetDeploymentContainers(deploymentItem)
			initContainers := callbacks.GetDeploymentInitContainers(deploymentItem)
			...
			volumes := callbacks.GetDeploymentVolumes(deploymentItem)
